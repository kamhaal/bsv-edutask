...
function TaskDetail({ taskid, updateTasks }) {
  ...
  const updateTask = () => {
    fetch(
      `http://localhost:${process.env.REACT_APP_BACKEND_PORT}/tasks/byid/${taskid}`,
      {
        method: "get",
  ...
  const addTodo = (e) => {
    ...
    fetch(
      `http://localhost:${process.env.REACT_APP_BACKEND_PORT}/todos/create`,
      {
        method: "post",
        body: data,
        headers: { "Cache-Control": "no-cache" },
  ...
  const toggleTodo = (todo) => {
    ...
    fetch(
      `http://localhost:${process.env.REACT_APP_BACKEND_PORT}/todos/byid/${todo._id}`,
      {
        method: "put",
        body: data,
        headers: { "Cache-Control": "no-cache" },
        ...
        {todos.map((todo) => (
          <li key={todo._id} className="todo-item">
            ...
            <Editable
              ...
              updateTasks={updateTasks}
            />
            <span className="remover" onClick={() => deleteTodo(todo)}>
              &#x2716;
            </span>
          </li>
        ))}
